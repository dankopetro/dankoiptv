...
from dankoiptv_lib.xdg import get_cache_dir
...
class Channel:
    ...
    def __init__(self, xtream: object, group_title, stream_info):
        ...
        if stream_type not in ("live", "movie"):
            logger.error(
                f"Error the channel has unknown stream type `{stream_type}`\n`{stream_info}`"
            )
        else:
            # Raw JSON Channel
            ...
            # Required by Hypnotix
            self.url = (
                f"{xtream.server}/{stream_type}/{xtream.authorization['username']}/"
            ...
            if not xtream._validate_url(self.url):
                logger.warning(f"{self.name} - Bad URL? `{self.url}`")
...
class Group:
    ...
    def __init__(self, group_info: dict, stream_type: str):
        ...
        if "VOD" == stream_type:
            ...
            logger.warning(
                f"Unrecognized stream type `{stream_type}` for `{group_info}`"
            )
...
class XTream:
    ...
    def __init__(
        ...
        default_cache_path = str(Path(get_cache_dir(), "xtream"))
        ...
        if self.cache_path != "":
            ...
            if not osp.isdir(self.cache_path):
                logger.warning(
                    f" - Cache Path is not a directory, using default '{default_cache_path}'"
                )
                self.cache_path == ""
    ...
    def search_stream(
        self, keyword: str, ignore_case: bool = True, return_type: str = "LIST"
    ) -> List:
        """Search for streams
        ...
        logger.info(f"Checking {len(self.movies)} movies")
        for stream in self.movies:
            if re.match(regex, stream.name) is not None:
                search_result.append(stream.export_json())

        logger.info(f"Checking {len(self.channels)} channels")
        for stream in self.channels:
            if re.match(regex, stream.name) is not None:
                search_result.append(stream.export_json())

        logger.info(f"Checking {len(self.series)} series")
        ...
        if return_type == "JSON":
            if search_result is not None:
                logger.info(f"Found {len(search_result)} results `{keyword}`")
    ...
    def authenticate(self):
        ...
        if self.state["authenticated"] is False:
            ...
            #        r = requests_get(url, timeout=(4), headers=self.connection_headers)
            #        i = 31
            ...
            try:
                r = requests_get(url, timeout=(10), headers=self.connection_headers)
            except requests.exceptions.ConnectionError:
                r = None
            ...
            if r is not None:
                ...
                if r.ok:
                    ...
                    if "user_info" not in self.auth_data:
                        raise Exception(f"XTream auth failed: {json.dumps(self.auth_data)}")
                    self.authorization = {
                    ...
                    if "https_port" in self.auth_data["server_info"]:
                        self.base_url_ssl = (
                            f"https://{self.auth_data['server_info']['url']}:{self.auth_data['server_info']['https_port']}"
                            f"/player_api.php?username={self.username}&password={self.password}"
                        )
                else:
                    self.update_status(
                        f"{self.name}: Provider could not be loaded. Reason: `{r.status_code} {r.reason}`"
                    )
            else:
        ...
        if osp.isfile(full_filename):
            ...
            if self.threshold_time_sec > file_age_sec:
                ...
                try:
                    ...
                    logger.warning(
                        f" - Could not load from file `{full_filename}`: e=`{e}`"
                    )
            return my_data
    ...
    def _save_to_file(self, data_list: dict, filename: str) -> bool:
        ...
        if data_list is not None:
            ...
            try:
                ...
                logger.warning(f" - Could not save to file `{full_filename}`: e=`{e}`")
                return False
    ...
    def load_iptv(self) -> bool:
        ...
        if self.state["authenticated"] is False:
            logger.warning("Warning, cannot load streams since authorization failed")
            return False
        ...
        for loading_stream_type in (self.live_type, self.vod_type, self.series_type):
            ...
            if all_cat is None:
                ...
                if all_cat is not None:
                    self._save_to_file(
                        all_cat, f"all_groups_{loading_stream_type}.json"
                    )
            dt = timer() - start
            ...
            if all_cat is not None:
                ...
                logger.warning(f" - Could not load {loading_stream_type} Groups")
                break
            ...
            if all_streams is None:
                ...
                self._save_to_file(
                    all_streams, f"all_stream_{loading_stream_type}.json"
                )
            dt = timer() - start
                    ...
                    if not skip_stream:
                        ...
                        the_group = next(
                            (
                                ...
                                if x.group_id == int(stream_channel["category_id"])
                            ),
                            None,
                        )
                        ...
                        if new_channel.group_id == "9999":
                            logger.info(
                                f" - xEverythingElse Channel -> {new_channel.name} - {new_channel.stream_type}"
                            )
                        ...
                        if the_group is not None:
                            ...
                            logger.warning(
                                f" - Group not found `{stream_channel['name']}`"
                            )
                dt = timer() - start
                # Print information of which streams have been skipped
                if self.hide_adult_content:
                    logger.info(
                        f" - Skipped {skipped_adult_content} adult {loading_stream_type} streams"
                    )
                if skipped_no_name_content > 0:
                    logger.info(
                        f" - Skipped {skipped_no_name_content} unprintable {loading_stream_type} streams"
                    )
            else:
                logger.warning(f" - Could not load {loading_stream_type} Streams")
    ...
    def _save_to_file_skipped_streams(self, stream_channel: Channel):
        ...
        try:
            ...
            logger.warning(
                f" - Could not save to skipped stream file `{full_filename}`: e=`{e}`"
            )
        return False
    ...
    def _get_request(self, url: str, timeout: Tuple = (2, 15)):
        ...
        while i < 10:
            ...
            try:
                ...
                logger.error(
                    " - Connection Error: Possible network problem (e.g. DNS failure, refused connection, etc)"
                )
                i += 1
    ...
    def liveEpgByStreamAndLimit(self, stream_id, limit):
        return self._get_request(
            self.get_live_epg_URL_by_stream_and_limit(stream_id, limit)
        )
    ...
    def get_live_epg_URL_by_stream_and_limit(self, stream_id, limit) -> str:
        return (
            f"{self.base_url}&action=get_short_epg&stream_id={stream_id}&limit={limit}"
        )
    ...
    def get_all_epg_URL(self) -> str:
        return (
            f"{self.server}/xmltv.php?username={self.username}&password={self.password}"
        )
