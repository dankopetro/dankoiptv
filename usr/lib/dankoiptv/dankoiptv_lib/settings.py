...
from dankoiptv_lib.xdg import LOCAL_DIR, SAVE_FOLDER_DEFAULT
...
def get_epg_url():
    return (
        DankoData.settings["epg"]
        if DankoData.settings["epg"]
        else (
            DankoData.settings["epg_temporary"]
            if "epg_temporary" in DankoData.settings
            and DankoData.settings["epg_temporary"]
            else ""
        )
    )
