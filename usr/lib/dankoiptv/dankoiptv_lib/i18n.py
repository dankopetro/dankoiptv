...
locale.bindtextdomain("dankoiptv", LOCALE_DIR)
gettext.bindtextdomain("dankoiptv", LOCALE_DIR)
gettext.textdomain("dankoiptv")


def cached_gettext(gettext_str):
    if gettext_str not in DankoLang.cache_gettext:
        DankoLang.cache_gettext[gettext_str] = gettext.gettext(gettext_str)
    return DankoLang.cache_gettext[gettext_str]


def cached_ngettext(*args):
    if args not in DankoLang.cache_ngettext:
        DankoLang.cache_ngettext[args] = gettext.ngettext(*args)
    return DankoLang.cache_ngettext[args]
