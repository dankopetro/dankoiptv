...
from dankoiptv_lib.settings import parse_settings
...
def is_ffmpeg_recording():
    ret = -2
    if DankoData.ffmpeg_proc:
        if DankoData.ffmpeg_proc.processId() == 0:
            DankoData.ffmpeg_proc = None
            ret = True
        else:
            ret = False
    return ret
...
def exit_handler(process, exit_code, _exit_status):
    is_killed = False
    try:
        is_killed = process._danko_killed
    except Exception:
        pass
    if not is_killed:
        logger.warning("ffmpeg process crashed")
        if DankoData.show_record_exception:
            ...
            try:
                ...
                pass
            DankoData.show_record_exception(
                _("ffmpeg crashed!") + "\n"
...
def record(
    ...
    if not is_return:
        DankoData.ffmpeg_proc = QtCore.QProcess()
        DankoData.ffmpeg_proc.start("ffmpeg", arr)
        DankoData.ffmpeg_proc.finished.connect(
            lambda exit_code, exit_status: exit_handler(
                DankoData.ffmpeg_proc, exit_code, exit_status
            )
        )
    else:
...
def terminate_record_process(proc):
    proc._danko_killed = True
    proc.kill()


def stop_record():
    if DankoData.ffmpeg_proc:
        terminate_record_process(DankoData.ffmpeg_proc)


def init_record(show_exception, ffmpeg_processes):
    DankoData.show_record_exception = show_exception
    DankoData.ffmpeg_processes = ffmpeg_processes
