...
from dankoiptv_lib.xdg import LOCAL_DIR
from dankoiptv_lib.misc import DankoData
from dankoiptv_lib.xtream import load_xtream
...
class PlaylistsWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.name_label.setFont(DankoData.DankoGUI.font_bold)
        self.description_label = QtWidgets.QLabel()
        self.description_label.setFont(DankoData.DankoGUI.font_italic)
        self.description_label.setWordWrap(True)

        self.icon_label = QtWidgets.QLabel()
        self.icon_label.setPixmap(DankoData.DankoGUI.tv_icon)
        ...
        self.btn_select.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "select.png")))
        )
        ...
        self.btn_edit.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "edit_playlist.png")))
        )
        ...
        self.btn_delete.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "trash.png")))
        )
...
class PlaylistsWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(DankoData.DankoGUI.main_icon)

        self.dankoiptv_lib_icon = QtWidgets.QLabel()
        self.dankoiptv_lib_icon.setPixmap(DankoData.DankoGUI.tv_icon)

        self.dankoiptv_lib_label = QtWidgets.QLabel()
        self.dankoiptv_lib_label.setFont(DankoData.DankoGUI.font_11_bold)
        self.dankoiptv_lib_label.setTextFormat(QtCore.Qt.TextFormat.RichText)
        danko_color = "#1b1c1c;"
        if DankoData.use_dark_icon_theme:
            danko_color = "white"
        self.dankoiptv_lib_label.setText(
            f'&nbsp;<span style="color: {danko_color};">dankoiptv</span>'
        )

        self.dankoiptv_lib = QtWidgets.QWidget()
        self.dankoiptv_lib_layout = QtWidgets.QHBoxLayout()
        self.dankoiptv_lib_layout.addWidget(self.dankoiptv_lib_icon)
        self.dankoiptv_lib_layout.addWidget(self.dankoiptv_lib_label)
        self.dankoiptv_lib.setLayout(self.dankoiptv_lib_layout)

        self.playlists_add = QtWidgets.QPushButton()
        self.playlists_add.setToolTip(_("Add playlist"))
        self.playlists_add.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "add.png")))
        )
        ...
        #     QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "star.png")))
        # )
        ...
        self.playlists_settings.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "settings.png")))
        )
        self.playlists_settings.clicked.connect(lambda: DankoData.show_settings())
        ...
        self.playlists_layout.addWidget(self.dankoiptv_lib)
        self.playlists_layout.setAlignment(
            self.dankoiptv_lib, QtCore.Qt.AlignmentFlag.AlignHCenter
        )
        ...
        self.search_icon.setPixmap(DankoData.DankoGUI.search_icon_pixmap_small)
        ...
        danko_color_2 = "#424242"
        if DankoData.use_dark_icon_theme:
            danko_color_2 = "white"
        self.playlists_notice_label.setStyleSheet(
            f"color: {danko_color_2}; font-size: 0.7em;"
        )
    ...
    def dropEvent(self, event: QtGui.QDropEvent):
        ...
        for url in file_urls:
            ...
            if True:  # os.path.isfile(file_path):
                ...
                if name in DankoData.playlists_saved:
                    name = f"{name} "
                DankoData.playlists_saved[name] = playlist
...
def do_search():
    DankoData.playlists_search = Data.playlists_win.search_edit.text()
    populate_playlists()
...
def favourites_plus():
    ...
    DankoData.DankoGUI.m3u = str(Path(LOCAL_DIR, "favplaylist.m3u"))
    DankoData.DankoGUI.epg = ""
    DankoData.DankoGUI.soffset.setValue(0)
    DankoData.save_settings()
...
class Playlists_Edit(QtWidgets.QMainWindow):
    def __init__(self, overwrite=False, overwrite_name=""):
        ...
        self.path_file.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "file.png")))
        )
        ...
        self.useragent_edit.setPlaceholderText(DankoData.settings["ua"])

        self.referer_label = QtWidgets.QLabel(_("HTTP Referer:"))
        self.referer_edit = QtWidgets.QLineEdit()
        self.referer_edit.setPlaceholderText(DankoData.settings["referer"])
        ...
        self.save_btn.setFont(DankoData.DankoGUI.font_bold)
        ...
        self.udp_proxy_edit.setPlaceholderText(DankoData.settings["udp_proxy"])
        ...
        def more_settings_clicked():
            ...
            if self.more_settings_shown:
                self.more_settings.setIcon(DankoData.DankoGUI.minus_icon)
            else:
                self.more_settings.setIcon(DankoData.DankoGUI.plus_icon)
            redraw_layout()
    ...
    def on_save(self, overwrite=False):
        ...
        if not url:
            ...
            if not overwrite and name in DankoData.playlists_saved:
                name = f"{name} "
            if self.old_name:
                DankoData.playlists_saved.pop(self.old_name)
            DankoData.playlists_saved[name] = playlist
...
def save_playlists():
    with open(
        ...
        playlist_file.write(json.dumps(DankoData.playlists_saved))
...
def xtream_playlists_set_icon(icon):
    for _i in range(Data.playlists_win.playlists_list.count()):
        ...
        if _internal_name.startswith("playlist:"):
            ...
            if _internal_name in DankoData.playlists_saved:
                if DankoData.playlists_saved[_internal_name]["m3u"] in XTream_list.data:
...
def show_xtream_playlists_expiration_set():
    for _i in range(Data.playlists_win.playlists_list.count()):
        ...
        if _internal_name.startswith("playlist:"):
            _internal_name = _internal_name.replace("playlist:", "", 1)
            if _internal_name in DankoData.playlists_saved:
                path = DankoData.playlists_saved[_internal_name]["m3u"]
...
def xtream_playlists_show_loading():
    xtream_playlists_set_icon(DankoData.DankoGUI.loading_icon_small)


def xtream_playlists_hide_loading():
    xtream_playlists_set_icon(DankoData.DankoGUI.tv_icon)


def show_xtream_playlists_expiration():
    try:
        if not DankoData.xtream_list_lock:
            DankoData.xtream_list_lock = True
            xtream_data = []
            for playlist in DankoData.playlists_saved:
                path = DankoData.playlists_saved[playlist]["m3u"]
                if path.startswith("XTREAM::::::::::::::"):
                    xtream_data.append([path, DankoData.playlists_saved[playlist]])
            if xtream_data != XTream_list.data:
                ...
                for xt_name in XTream_list.data:
                    ...
                    if "User-Agent" not in headers:
                        headers["User-Agent"] = DankoData.settings["ua"]
                    if "Referer" not in headers:
                        headers["Referer"] = DankoData.settings["referer"]
            ...
            DankoData.xtream_list_lock = False
    except Exception:
...
def edit_playlist(playlist_name):
    if playlist_name in DankoData.playlists_saved:
        win = Playlists_Edit(DankoData.playlists_saved[playlist_name], playlist_name)
        Data.windows.append(win)
        win.show()
    else:
...
def delete_playlist(playlist_name):
    ...
    if is_delete == QtWidgets.QMessageBox.StandardButton.Yes:
        DankoData.playlists_saved.pop(playlist_name)
        save_playlists()
        populate_playlists()
...
def playlist_selected(sel):
    ...
    if sel.startswith("internal:"):
        ...
        if sel in DankoData.playlists_saved:
            playlist = DankoData.playlists_saved[sel]
            ...
            Data.playlists_win.close()
            DankoData.DankoGUI.m3u = path
            DankoData.DankoGUI.epg = epg
            DankoData.DankoGUI.soffset.setValue(epg_offset)
            DankoData.settings["playlist_useragent"] = useragent
            DankoData.settings["playlist_referer"] = referer
            DankoData.settings["playlist_udp_proxy"] = udp_proxy
            DankoData.save_settings()
        else:
    ...
    if DankoData.playlists_saved:
        ...
        with open(Path(LOCAL_DIR, "favplaylist.m3u"), "r") as favplaylist:
            if favplaylist.read() != "#EXTM3U\n#EXTINF:-1,-\nhttp://255.255.255.255\n":
                ...
                playlist_widget.icon_label.setPixmap(DankoData.DankoGUI.star_icon_pixmap)
                item.setSizeHint(
                    QtCore.QSize(
                        Data.playlists_win.playlists_list.sizeHint().width(),
        ...
        for playlist_name in DankoData.playlists_saved:
            if (
                DankoData.playlists_search.lower().strip()
            ...
            if "added" in DankoData.playlists_saved[playlist_name]:
                added_time = datetime.datetime.fromtimestamp(
                    DankoData.playlists_saved[playlist_name]["added"]
                ).strftime("%c")
                playlist_widget.description_label.setText(
                    _("Added") + f": {added_time}"
                )
            if DankoData.playlists_saved[playlist_name]["m3u"] in XTream_list.expiration:
                playlist_widget.description_label.setText(
                    get_xtream_expiration_text(
                        DankoData.playlists_saved[playlist_name]["m3u"]
                    )
                )
            playlist_widget.btn_select.clicked.connect(
        ...
        if Data.playlists_win.playlists_list.count() == 0:
            ...
            playlist_widget.btn_edit.hide()
            playlist_widget.btn_delete.hide()
            playlist_widget.icon_label.setPixmap(DankoData.DankoGUI.search_icon_pixmap)
            item.setSizeHint(
                QtCore.QSize(
                    Data.playlists_win.playlists_list.sizeHint().width(),
                    playlist_widget.sizeHint().height(),
