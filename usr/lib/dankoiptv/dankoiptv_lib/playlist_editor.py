...
class PlaylistEditor(QtWidgets.QMainWindow):
    ...
    def select_file(self):
        ...
        if filename:
            m3u_parser = M3UParser(DankoData.settings["udp_proxy"])
            try:
                file0 = open(filename)
                filedata = file0.read()
                file0.close()
    ...
    def populate_toolbar(self):
        ...
        add_btn.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "plus.png")))
        )
        ...
        down_btn.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "arrow-down.png")))
        )
        down_btn.clicked.connect(lambda: self.move_row(1))

        up_btn = QtWidgets.QToolButton()
        up_btn.setIcon(
            QtGui.QIcon(str(Path(DankoData.DankoGUI.icons_folder, "arrow-up.png")))
        )
