...
def mpris_handle_method_call(
    ...
    if interface_name == "org.freedesktop.DBus.Properties" and method_name == "Get":
        ...
        invocation.return_value(
            DankoData.callback((interface_name, method_name, params))
        )
...
def start_mpris(pid, callback, get_options):
    DankoData.callback = callback
    DankoData.get_options = get_options
    return gi.repository.Gio.bus_own_name(
        gi.repository.Gio.BusType.SESSION,
        f"org.mpris.MediaPlayer2.dankoiptv_lib.instance{pid}",
...
def emit_mpris_change(interface_name, variable):
    if DankoData.mpris_bus is None:
        try:
            DankoData.mpris_bus = gi.repository.Gio.bus_get_sync(
                gi.repository.Gio.BusType.SESSION, None
            )
        except Exception:
            pass
    try:
        DankoData.mpris_bus.emit_signal(
            None,
            "/org/mpris/MediaPlayer2",
