...
from dankoiptv_lib.xdg import SAVE_FOLDER_DEFAULT
from dankoiptv_lib.multi_epg import MultiEPGWindow
from dankoiptv_lib.qt_exception import show_exception
from dankoiptv_lib.misc import DankoData, WINDOW_SIZE
from dankoiptv_lib.skins import (
    SKIN_NAMES,
    APP_FONTS,
    resolve_font_family,
    app_font,
...
def move_window_to_center(window, from_window=None):
    screen = (
        ...
        if from_window
            ...
            DankoData.DankoGUI.win.screen()
            if DankoData.DankoGUI.win
...
class DankoGUIClass:
    ...
    def __init__(self):
        self.icons_folder = str(
            Path(
                ...
                "share",
                "dankoiptv",
                "icons" if not DankoData.use_dark_icon_theme else "icons_dark",
            )
        )
        ...
        class ScrollableLabel(QtWidgets.QScrollArea):
            def __init__(self, *args, **kwargs):
                ...
                bcolor_scrollabel = (
                    "white" if not DankoData.use_dark_icon_theme else "black"
                )
        ...
        # Dankoiptv: fuente Ubuntu Medium (casi bold) con fallback Noto Sans
        try:
            _fam = resolve_font_family(
                DankoData.settings.get("font_family", "Ubuntu")
            )
        except Exception:
            _fam = "Ubuntu"

        self.font_bold = QtGui.QFont(_fam)
        try:
            self.font_bold.setWeight(QtGui.QFont.Weight.Medium)
        except Exception:
            self.font_bold.setBold(True)

        self.font_bold_medium = QtGui.QFont(_fam)
        self.font_bold_medium.setPointSize(11)
        try:
            self.font_bold_medium.setWeight(QtGui.QFont.Weight.Medium)
        except Exception:
            self.font_bold_medium.setBold(True)

        self.font_italic = QtGui.QFont(_fam)
        self.font_italic.setItalic(True)

        self.font_italic_medium = QtGui.QFont(_fam)
        self.font_italic_medium.setPointSize(11)
        self.font_italic_medium.setItalic(True)

        self.font_11_bold = QtGui.QFont(_fam)
        self.font_11_bold.setPointSize(11)
        try:
            self.font_11_bold.setWeight(QtGui.QFont.Weight.Medium)
        except Exception:
            self.font_11_bold.setBold(True)

        self.font_12 = QtGui.QFont(_fam)
        self.font_12.setPointSize(12)

        self.font_12_bold = QtGui.QFont(_fam)
        self.font_12_bold.setPointSize(12)
        try:
            self.font_12_bold.setWeight(QtGui.QFont.Weight.Medium)
        except Exception:
            self.font_12_bold.setBold(True)

        class PlaylistWidget(QtWidgets.QWidget):
            def __init__(self, DankoGUI):
                super().__init__()

                self.name_label = QtWidgets.QLabel()
                self.name_label.setFont(DankoGUI.font_bold)
                ...
                if not DankoData.settings["hidechannellogos"]:
        ...
        self.license_str = (
            ...
            "\n"
            "dankoiptv is based on Astroncia IPTV code.\n"
            "\n"
        ...
        # Dankoiptv: selector de tema (skin) + fuente en Ajustes/GUI
        self.skin_label = QtWidgets.QLabel("{}:".format(_("Theme (skin)")))
        self.skin_choose = QtWidgets.QComboBox()
        for _skin_name in SKIN_NAMES:
            self.skin_choose.addItem(_skin_name)
        self.skin_choose.currentTextChanged.connect(self._on_skin_changed)

        self.appfont_label = QtWidgets.QLabel("{}:".format(_("Application font")))
        self.font_choose = QtWidgets.QComboBox()
        for _font_name in APP_FONTS:
            self.font_choose.addItem(_font_name)
        self.font_choose.currentTextChanged.connect(self._on_font_changed)
        ...
        self.tab_gui.layout.addWidget(self.skin_label, 9, 0)
        self.tab_gui.layout.addWidget(self.skin_choose, 9, 1)
        self.tab_gui.layout.addWidget(self.appfont_label, 10, 0)
        self.tab_gui.layout.addWidget(self.font_choose, 10, 1)
    ...
    def create_windows(self):
        ...
        self.help_win.setWindowTitle(_("&About Dankoiptv").replace("&", ""))
    ...
    def get_settings(self):
        ...
        settings_arr = {
            ...
            "playlist_useragent": DankoData.settings["playlist_useragent"],
            "playlist_referer": DankoData.settings["playlist_referer"],
            "playlist_udp_proxy": DankoData.settings["playlist_udp_proxy"],
            "skin": self.skin_choose.currentText(),
            "font_family": self.font_choose.currentText(),
        }

        return settings_arr

    def _on_skin_changed(self, _name):
        # Aplicar skin en caliente (también se guarda con Save)
        try:
            DankoData.settings["skin"] = self.skin_choose.currentText()
            apply_look()
        except Exception:
            pass

    def _on_font_changed(self, _name):
        # Aplicar fuente en caliente (también se guarda con Save)
        try:
            DankoData.settings["font_family"] = self.font_choose.currentText()
            apply_look()
        except Exception:
            pass
    ...
    def create4(self):
        ...
        class StreaminfoWin(QtWidgets.QMainWindow):
            def showEvent(self, _event):
                DankoData.streaminfo_win_visible = True

            def hideEvent(self, _event):
                DankoData.streaminfo_win_visible = False
    ...
    def set_from_settings(self):
        self.m3u = DankoData.settings["m3u"]
        self.epg = DankoData.settings["epg"]
        self.udp_proxy_widget.setText(DankoData.settings["udp_proxy"])
        self.deinterlace_widget.setChecked(DankoData.settings["deinterlace"])
        self.save_folder_widget.setText(DankoData.settings["save_folder"])
        self.soffset.setValue(DankoData.settings["epgoffset"])
        self.scache1.setValue(DankoData.settings["cache_secs"])
        self.referer_choose.setText(DankoData.settings["referer"])
        self.useragent_choose_2.setText(DankoData.settings["ua"])
        self.mpv_options.setText(DankoData.settings["mpv_options"])
        self.donot_flag.setChecked(DankoData.settings["donotupdateepg"])
        self.openprevchannel_flag.setChecked(DankoData.settings["openprevchannel"])
        self.hideepgpercentage_flag.setChecked(DankoData.settings["hideepgpercentage"])
        self.hideepgfromplaylist_flag.setChecked(
            DankoData.settings["hideepgfromplaylist"]
        )
        self.hidebitrateinfo_flag.setChecked(DankoData.settings["hidebitrateinfo"])
        self.volumechangestep_choose.setValue(DankoData.settings["volumechangestep"])
        self.panelposition_choose.setCurrentIndex(DankoData.settings["panelposition"])
        self.autoreconnection_flag.setChecked(DankoData.settings["autoreconnection"])
        self.channellogos_select.setCurrentIndex(DankoData.settings["channellogos"])
        self.nocacheepg_flag.setChecked(DankoData.settings["nocacheepg"])
        self.scrrecnosubfolders_flag.setChecked(DankoData.settings["scrrecnosubfolders"])
        self.hidetvprogram_flag.setChecked(DankoData.settings["hidetvprogram"])
        self.sort_widget.setCurrentIndex(DankoData.settings["sort"])
        self.sort_categories_widget.setCurrentIndex(
            DankoData.settings["sort_categories"]
        )
        self.description_view_widget.setCurrentIndex(
            DankoData.settings["description_view"]
        )
        ...
        self.videoaspect_def_choose.setCurrentIndex(DankoData.settings["videoaspect"])
        self.zoom_def_choose.setCurrentIndex(DankoData.settings["zoom"])
        self.panscan_def_choose.setValue(DankoData.settings["panscan"])
        self.rewindenable_flag.setChecked(DankoData.settings["rewindenable"])
        self.hidechannellogos_flag.setChecked(DankoData.settings["hidechannellogos"])
        self.enabletransparency_flag.setChecked(DankoData.settings["enabletransparency"])
        self.hideplaylistbyleftmouseclick_flag.setChecked(
            DankoData.settings["hideplaylistbyleftmouseclick"]
        )
        # Dankoiptv: restaurar skin + fuente guardadas
        try:
            _saved_skin = DankoData.settings.get("skin", SKIN_NAMES[0])
            _idx = self.skin_choose.findText(_saved_skin)
            self.skin_choose.blockSignals(True)
            self.skin_choose.setCurrentIndex(_idx if _idx >= 0 else 0)
            self.skin_choose.blockSignals(False)
        except Exception:
            pass
        try:
            _saved_font = DankoData.settings.get("font_family", APP_FONTS[0])
            _fidx = self.font_choose.findText(_saved_font)
            self.font_choose.blockSignals(True)
            self.font_choose.setCurrentIndex(_fidx if _fidx >= 0 else 0)
